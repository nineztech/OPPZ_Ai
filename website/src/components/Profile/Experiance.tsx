...
const ExperienceManager: React.FC = () => {
  ...
  // Load saved experiences from localStorage on component mount
  useEffect(() => {
    const saved = localStorage.getItem('workExperiences');
    if (saved) {
      try {
        const parsedExperiences = JSON.parse(saved);
        setSavedExperiences(parsedExperiences);
      } catch (error) {
  ...
  const saveExperiences = () => {
    ...
    // Combine with existing saved experiences
    const allSavedExperiences = [...savedExperiences, ...validExperiences];
    
    // Save to localStorage
    localStorage.setItem('workExperiences', JSON.stringify(allSavedExperiences));
    
    // Update state
    setSavedExperiences(allSavedExperiences);
  ...
  const deleteSavedExperience = (id: string) => {
    const updatedExperiences = savedExperiences.filter(exp => exp.id !== id);
  ...
  return (
    <div className="max-w-4xl mx-auto  p-6 bg-gray-100 min-h-screen">
      <div className="mb-8 ">
        <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-green-500 via-blue-600 to-purple-700">Work Experience</h1>
      ...
      <div className="mb-8">
        ...
        <div className="space-y-4">
          {experiences.map((experience, index) => (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white text-black px-3 py-2 rounded-lg border border-blue-500 focus:border-blue-400 focus:outline-none transition-colors"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white text-black px-3 py-2 rounded-lg border border-gray-600 focus:border-blue-500 focus:outline-none transition-colors"
              ...
              {/* Currently work here checkbox and Remove button */}
        ...
        <div className="mt-8">
          <button
            ...
            className="w-full bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-medium py-3 px-6 rounded-lg transition-all duration-200 transform hover:scale-[1.02]"
          >
            Save Experiences
          </button>
        </div>
        ...
        <div className="mt-12 p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              className="text-red-400 bg-red-100 h-10 w-18 p-2 border border-red-300 rounded-xl hover:text-red-300 text-sm font-medium transition-colors"
            >
              Clear All
            </button>
          </div>
          ...
          <div className="space-y-4 mb-8">
            {savedExperiences.map((experience) => (
              <div
                key={experience.id}
                className="bg-gray-20 shadow rounded-lg p-6 border  hover:border-gray-600 transition-colors"
              >
