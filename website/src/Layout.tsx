    ...
    <div className="hidden md:flex items-center space-x-6 mr-8">
      ...
      <Link to="/signup" className="text-gray-100 hover:text-blue-100 hover:border-b-2 border-white">Signup</Link>
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center ml-8 space-x-2 cursor-pointer mr-56">
      {/* <img src="/OPPZ_Ai_Logo.png" alt="Logo" width="40" height="40" /> */}
      <span className="text-xl font-bold ml-8 text-gray-100">"OPPZ Ai – Auto-Apply to Jobs on LinkedIn, Effortlessly."</span>
    </div>
                <div className="hidden sm:block justify-end text-right">
                  <p className="text-sm text-gray-100">Welcome back,</p>
                  <p className="text-sm font-semibold text-gray-100">{user?.firstname}!</p>
                </div>
                <Link to="/UserProfile" className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white font-semibold">
                  {user?.firstname?.[0]?.toUpperCase() || 'U'}
                </Link>
                <button onClick={handleLogout} className="px-3 py-2 text-sm bg-red-500 text-white hover:bg-red-600 rounded">Logout</button>
              </div>
            )}
          </div>
        </nav>
